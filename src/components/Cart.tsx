...
export const Cart = () => {
    ...
    return (
        <div>
            <p>Produkty:</p>
            {products.map(product => <div key={product.name}>{product.name}<button onClick={() => dispatch({ type: "increment", payload: { id: product.id } })}>Increment</button></div>)}

            <p>Koszyk:</p>
            {state.map(product => <div key={product.id}> <button onClick={() => dispatch({ type: "decrement", payload: { id: product.id } })}>Decrement</button>{product.name}<button onClick={() => dispatch({ type: "increment", payload: { id: product.id } })}>Increment</button></div>)}
